...
def metrics(im_pattern, label_dir, loss_fn):
   ...
   # 自动识别数据集类型
    if 'sice' in im_pattern.lower():
        testset = 'SICE'
    elif 'lol-v2r' in im_pattern.lower():
        testset = 'LOL-v2R'
    else:
        testset = 'LOL'
    ...
    for item in image_paths:
        ...
        # === GT 文件名获取逻辑 ===
        if testset == 'LOL' or testset == 'LOL-v2S':
            name = os.path.basename(item)

        elif testset == 'SICE':
            name = os.path.basename(item).split('_')[0] + '.JPG'

        elif testset == 'LOL-v2R':
...
if __name__ == '__main__' :
    ...
    im_dir = './results/IUJDE/LOL-v2R/I/*.png'
    label_dir = './dataset/test/LOL-v2R/reference'
